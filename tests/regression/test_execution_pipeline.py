...
def test_every_non_trading_execution_outcome_reaches_a_terminal_state(
    fill_status: FillStatus, expected_status: OrderStatus
) -> None:
    """No execution outcome may leave an accepted order open with no fills.

    v2.1 closed this for REJECTED and EXPIRED but left NO_FILL behind: the order
    stayed ACCEPTED in oms.active_orders forever and its allocation reservation
    was never released, so open orders never reconciled with fills.
    """
    ...
    result = ExecutionPipeline.process_quote(state, _quote(asset_id), _context_factory, fill_status)
    ...
    # No fill, trade or position may be fabricated.
    assert order.filled_quantity == Decimal("0")
    assert result.execution_reports == ()
    assert result.fills == ()
    assert result.trades == ()
    ...
    releases = [
        e
        for e in result.state.allocation.events
        if type(e).__name__ == "AllocationReservationReleased"
    ]
