...
def sample_payload() -> ResearchPayload:
    ...
    trades = tuple(
        TradePayload(f"T{i}", "AAPL", 100.0, 105.0, 10.0, 50.0, 86400.0)
        for i in range(100)
    )
...
def test_bias_overfitting_risk() -> None:
    ...
    payload = ResearchPayload("S", (), trades, {"a":1,"b":2,"c":3,"d":4,"e":5}, (), 1_000_000.0)
    report = detect_bias(payload)
...
def test_parameter_robustness_cliff() -> None:
    payload = ResearchPayload("S", (), (), {"a":1,"b":2,"c":3,"d":4,"e":5,"f":6}, (), 1_000_000.0)
    report = parameter_robustness(payload)
