...
def generic_metadata() -> DatasetMetadata:
    return DataAdapter.create_metadata(
        "AAPL-1D", "Yahoo", "EQUITY", "DAILY", 1000.0, 2000.0
    )
...
def test_parse_raw_rows_perfect() -> None:
    raw = [
        {"Date": 1000.0, "Open": 10, "High": 12, "Low": 9, "Close": 11, "Vol": 100}
    ]
...
def test_engine_ingest(
        base_state: UniversalDataState, 
        generic_metadata: DatasetMetadata
    ) -> None:
    raw = [{"timestamp": 1000.0, "o": 10, "h": 12, "l": 9, "c": 11, "v": 100}]
    ds = UniversalDataEngine.load(generic_metadata, raw)
