...
This directory contains the synthetic datasets used by the AlphaLab example suite. Examples `01`–`10` and these datasets date from **v1.0.0** and exercise the standalone engine APIs; `11_unified_backtest.py` (v2.2) drives the integrated execution path and builds its own in-memory dataset, `12_model_lifecycle.py` (v2.4) drives the model/strategy lifecycle path — research candidate to deployment and back — over a real backtest, and `13_durable_run_state.py` (v2.13) stops a seeded run, stores it with `FileRunStateStore`, continues it in a separate interpreter, and checks the result byte-for-byte against a run that never stopped. The last four build their own data and use none of the datasets here. `14_multi_currency_settlement.py` (v2.17) drains an FX feed into a run that settles two currencies and reports in one, and builds its own data too. `05_broker_connection.py` was rewritten in v2.17 against the canonical broker boundary, having used `alphalab.integrations` until v2.17 removed that package. None of the scripts are part of the automated test suite. All fourteen run; `03_replay.py`'s empty-dataset failure was fixed in v2.2.
