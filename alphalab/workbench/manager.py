...
class WorkbenchManager:
    ...
    def close_project(state: WorkbenchState, ts: float) -> WorkbenchState:
        ...
        return replace(state, active_project_id=None, tabs=new_tabs, events=(*state.events, evt))
    ...
    def close_tab(state: WorkbenchState, tab_id: str, ts: float) -> WorkbenchState:
        ...
        if was_active and new_tabs:
            last = new_tabs[-1]
            new_tabs = tuple(replace(t, is_active=(t.tab_id == last.tab_id)) for t in new_tabs)
