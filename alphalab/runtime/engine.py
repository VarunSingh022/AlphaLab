...
class RuntimeEngine:
    ...
    def start(state: RuntimeState, timestamp: float) -> RuntimeState:
        ...
        start_evt = RuntimeStarted(RuntimeEngine._create_id(), timestamp, state.runtime_id)
    ...
    def pause(state: RuntimeState, timestamp: float) -> RuntimeState:
        ...
        pause_evt = RuntimePaused(RuntimeEngine._create_id(), timestamp, state.runtime_id)
        return replace(state, status=RuntimeStatus.PAUSED, events=(*state.events, pause_evt))

    @staticmethod
    def resume(state: RuntimeState, timestamp: float) -> RuntimeState:
        validate_transition(state.status, RuntimeStatus.RUNNING)
        resume_evt = RuntimeResumed(RuntimeEngine._create_id(), timestamp, state.runtime_id)
        # Re-sync heartbeat
    ...
    def stop(state: RuntimeState, timestamp: float) -> RuntimeState:
        ...
        stop_evt = RuntimeStopped(RuntimeEngine._create_id(), timestamp, state.runtime_id)
        return replace(s1, status=RuntimeStatus.STOPPED, events=(*state.events, stop_evt))
    ...
    def fail(state: RuntimeState, reason: str, timestamp: float) -> RuntimeState:
        ...
        fail_evt = RuntimeFailed(RuntimeEngine._create_id(), timestamp, state.runtime_id, reason)
