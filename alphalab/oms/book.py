...
class OrderBook:
    ...
    def add(self, order: Order) -> "OrderBook":
        ...
        An order that declares no owning strategy is not entered in the strategy
        index. A netted order represents several strategies, so it has no single
        owner to index it under, and inventing one is what
        :func:`orders_for_strategy` used to answer with. The order itself is
    ...
    def remove(self, order_id: OrderId) -> "OrderBook":
        ...
        # Symmetric with ``add``: an order that was never indexed has no entry
        # to discard, and ``self._by_strategy[""]`` would raise.
        by_strategy = self._by_strategy
        if order.strategy_id:
            strategies = by_strategy[order.strategy_id].discard(order_id)
            by_strategy = (
                by_strategy.delete(order.strategy_id)
                if not strategies
                else by_strategy.set(order.strategy_id, strategies)
            )
    ...
    def orders_for_strategy(self, strategy_id: str) -> Sequence[Order]:
        ...
        Only orders that declare a single owning strategy are indexed. An order
        produced by :class:`~alphalab.allocation.engine.AllocationEngine` declares
        none -- it may represent several strategies at once, and a single-valued
        index cannot answer for it -- so a pipeline-produced order is not
        returned here by any key. Read
        :attr:`~alphalab.core.order_request.OrderRequest.contributions` for
        attribution. See ADR-0015 decision 4.
        """
